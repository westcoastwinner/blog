...
## 十、UNION  VS   UNION ALL

**`UNION`** 和 **`UNION ALL`** 都是 SQL 中用于合并多个 `SELECT` 语句结果集的运算符，但它们之间有一个关键的区别：是否去除重复行。

### 1. UNION 的功能

**`UNION`** 运算符用于合并两个或多个 `SELECT` 语句的结果集，并自动**去除**所有重复的行。

#### 特点

- **去重 (Distinct)**：这是它的核心功能。它会对合并后的结果进行排序和比较，只保留唯一的行。
- **性能开销**：由于需要对所有行进行去重操作，`UNION` 通常比 `UNION ALL` 慢，需要消耗更多的系统资源（例如 CPU 和临时空间）。

#### 什么时候用？

当你需要将来自不同表或不同查询的结果合并在一起，并且**确保最终结果中没有重复的记录**时，就使用 `UNION`。

**示例：**

```sql
SELECT employee_name, '正式员工' AS status FROM full_time_employees
UNION
SELECT employee_name, '兼职员工' AS status FROM part_time_employees;
-- 结果中不会有任何重复的员工姓名行。
```
...
**`UNION ALL`** 运算符用于合并两个或多个 `SELECT` 语句的结果集，但它**保留**所有重复的行。

#### 特点

- **保留重复 (All)**：它只是简单地将所有结果集堆叠在一起，不会进行去重处理。
- **性能高效**：因为它不需要进行耗时的排序和去重操作，所以 `UNION ALL` 通常比 `UNION` **快得多**。

#### 什么时候用？

当你需要合并多个结果集，并且：

1. 你知道结果集中**本身就不会有重复数据**（例如，合并来自不同、不交叉的表）。
2. 你需要**保留**所有数据，包括重复的记录（例如，计算总行数，或者记录的每次出现都很重要）。
...
-- 即使某个 product_id 在两个季度都有销售记录，它也会出现两次。
```
...
| 特性     | UNION                                  | UNION ALL                                              |
| -------- | -------------------------------------- | ------------------------------------------------------ |
| **去重** | **是** (自动去除所有重复行)            | **否** (保留所有行，包括重复行)                        |
| **性能** | 较慢（因为需要去重和排序）             | **较快**（只是简单堆叠）                               |
| **用途** | 需要一个**干净、无重复**的合并列表时。 | 当你**不关心重复**或**确定没有重复**时，用于提升性能。 |

#### 使用时的必要条件

无论是使用 `UNION` 还是 `UNION ALL`，所有被合并的 `SELECT` 语句都必须满足以下两个条件：

1. **列数（Number of Columns）**：每个查询中选择的列数必须**相同**。
2. **数据类型（Data Types）**：对应列的数据类型必须**兼容**（例如，不能将日期列与数字列合并）。

**最佳实践是：** 如果你不需要去重，**始终优先使用 `UNION ALL`**，因为它可以显著提高查询效率。只有当你确实需要去重时，才使用 `UNION`。
...
**`NOW()`** 函数返回当前系统日期和时间。它通常以 `'YYYY-MM-DD HH:MM:SS'` 的格式返回一个 **`DATETIME`** 值。

### 一般用法

它常用于：

- **记录创建或更新时间**：将当前时间插入到数据库表的某个时间字段中。
  - **示例:** `INSERT INTO logs (action, log_time) VALUES ('User logged in', NOW());`
- **时间比较**：作为比较的基准点，就像在您提供的代码中一样。
  - **示例:** `SELECT * FROM orders WHERE order_date < NOW();` (选择所有订单日期在当前时间之前的数据)
...
**`DATE_SUB(date, INTERVAL value unit)`** 函数从指定的日期（`date`）中减去一个时间间隔。

### 一般用法

它常用于：

- **计算过去的时间点**：例如，计算三天前、六个月前或一年前的日期。
- **定义时间范围**：在查询中设置一个动态的起始时间点，就像在您的代码中。

### 语法和参数解释

- **`date`**：必需。一个有效的日期或日期时间值（例如 `NOW()` 或表中的日期字段）。
- **`INTERVAL value unit`**：必需。定义要减去的时间间隔。
  - **`value`**：一个整数，表示要减去的数量。
  - **`unit`**：时间单位，例如 `DAY`, **`MONTH`**, `YEAR`, `HOUR`, `MINUTE`, `SECOND` 等。
...
`COALESCE()` 函数按顺序检查它所包含的表达式（或字段），并返回第一个 **非 NULL** 的值。

简单来说，它的作用就是提供一个或多个 **备用值**，以防第一个值是空的。
...
- 函数会从左到右依次检查 `expression1`, `expression2`, `expression3` 等。
- 只要找到一个 **非 NULL** 的值，它就会立即返回该值，并停止检查后续的表达式。
- 如果所有的表达式（参数）都是 **NULL**，那么 `COALESCE()` 函数最终会返回 **NULL**。

备注：参数可以是一个字段也可以是一个查询语句。



4.FIND_IN+**`FIND_IN_SET()`** 函数是 **MySQL** 数据库特有的一个非常实用的函数，用于在一个以逗号分隔的字符串列表中查找指定字符串的位置。
...
`FIND_IN_SET()` 用于确定一个特定的**子字符串**是否存在于一个用**逗号（`,`）** 分隔的字符串列表（**Set**）中，并返回它第一次出现的位置。

简单来说，它的作用就是在一个 "字符串数组" 中查找元素。
...
| 参数          | 描述                                     |
| ------------- | ---------------------------------------- |
| **`str`**     | 你想要查找的子字符串（要找的元素）。     |
| **`strlist`** | 以逗号分隔的字符串列表（被查找的集合）。 |

备注：strlist可以是手动写的集合，也可以是表中某个字段（这个字段以字符串存储且用,分隔）
